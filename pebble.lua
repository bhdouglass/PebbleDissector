...
lookup_endpoint = {
    ...
    [52] = {
        ...
        dissector = function(buffer, pinfo, tree)
            ...
            tree:add(buffer(0, 1), "Type: " .. typemsg)
            tree:add(buffer(1, 16), "UUID: " .. uuid)

            pinfo.cols.info = "APP_" .. typemsg
        end
    },
    [2000] = {
        name = "LOGS"
...
function pebble_proto.dissector(buffer, pinfo, tree)
    ...
    if not ep then
        pinfo.cols.info = "UNKNOWN (" .. endpoint .. ")"

        subtree:add(buffer(2, 2), "Endpoint: UNKNOWN (" .. endpoint .. ")")

        if datalen == len then
            subtree:add(buffer(4, len), "Data: " .. buffer(4, len))
        else
            pinfo.cols.info:append(" (ERROR: message length mismatch)")

            subtree:add(buffer(4, datalen), "Data: " .. buffer(4, datalen))
        end
        return
    end
    ...
    if datalen ~= len then
        pinfo.cols.info = ep.name .. " (ERROR: message length mismatch)"
